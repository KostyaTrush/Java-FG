...
public class ProductCompositeIntegration {
    ...
    public ResponseEntity<Product> getProduct(int productId){
        URI uri = util.getServiceUrl("product-service", "http://localhost:8081/product");
    ...
    public ResponseEntity<List<Recommendation>> getRecommendations(int productId){
        try {
            ...
            URI uri = util.getServiceUrl("recommendation-service", "http://localhost:8081/recommendation");
            String url = uri.toString() + "/recommendation?productId=" + productId;
            LOG.debug("GetReccomendations from URL: {}", url);
